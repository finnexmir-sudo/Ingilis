...
let mem = null;
const DEF = {p:{}, cfg:{dn:20, dr:60, ss:20, snd:1, goal:60, voice:""}, log:[], xp:0,
...
function sanitize(m){
  ...
  o.cfg = {
    ...
    snd:  c.snd ? 1 : 0,
    voice: (typeof c.voice === "string" && c.voice.length <= 120) ? c.voice : ""
  };
...
/* Ses keyfiyyetini bal ile qiymetlendirir: "compact" tipli zeif sesler
   asagi dusur, "neural/natural/google" tipli tebii sesler yuxari qalxir. */
function voiceScore(v){
  const nm = ((v && v.name) || "") + " " + ((v && v.voiceURI) || "");
  const lg = (v && v.lang) || "";
  let sc = 0;
  if (/natural|neural|enhanced|premium|siri/i.test(nm)) sc += 6;
  if (/google/i.test(nm)) sc += 5;
  if (/compact|espeak|pico|eloquence/i.test(nm)) sc -= 8;
  if (/^en[-_]GB/i.test(lg)) sc += 3;
  else if (/^en[-_]US/i.test(lg)) sc += 2;
  return sc;
}
const sameVoice = (v, id) => !!v && !!id && (v.voiceURI === id || v.name === id);
function pickVoices(){
  if (!("speechSynthesis" in window)) return;
  let list = [];
  try{ list = speechSynthesis.getVoices() || []; }catch(e){ list = []; }
  voices = list.filter(v => v && /^en/i.test(v.lang || ""))
               .map(v => ({v:v, sc:voiceScore(v)}))
               .sort((a,b) => b.sc - a.sc)
               .map(x => x.v);
  const want = cfg("voice");
  vA = (want ? voices.find(v => sameVoice(v, want)) : null) || voices[0] || null;
  vB = voices.find(v => v !== vA) || vA;
}
...
function say(t, o){
  ...
  try{
    if (sayT){ clearTimeout(sayT); sayT = null; }
    speechSynthesis.cancel();
    let text = String(t).replace(/\.\.\./g," ").trim();
    const rate = Math.max(0.5, o.rate || 0.9);
    /* Yavas rejimde her sozu ayri oxutmaq ucun aralarina vergul qoyulur. */
    if (rate <= 0.62 && /\s/.test(text)) text = text.split(/\s+/).join(", ");
    const v = o.voice === "b" ? vB : vA;
    /* cancel() ile speak() arasinda fasile olmasa Android ilk sozu kesir. */
    sayT = setTimeout(() => {
      sayT = null;
      try{
        const u = new SpeechSynthesisUtterance(text);
        u.lang = (v && v.lang) ? v.lang : "en-GB";
        u.rate = rate; u.pitch = o.pitch || 1; u.volume = 1;
        if (v) u.voice = v;
...
function dlgOpen(i){
  ...
  ses = {live:false, mode:"dlg", i, at:-1, playing:false, tr:true, rate:0.9};
...
function drawDlg(){
  ...
  render(`
    ...
    <div class="spd">
      <button onclick="dRate(0.55)" class="${ses.rate===0.55?"on":""}">Yavaş</button>
      <button onclick="dRate(0.9)" class="${ses.rate===0.9?"on":""}">Orta</button>
...
function dictCard(c, cb){
  let plays = 0, locked = false, rate = 0.85;
  const draw = () => render(`
    <div class="sh"><b>🎧</b><i>Cümləni dinlə və eşitdiyini yaz</i></div>
    <div class="spd" style="margin-top:14px">
      <button onclick="DR(0.55)" class="${rate===0.55?"on":""}">Yavaş</button>
      <button onclick="DR(0.85)" class="${rate===0.85?"on":""}">Orta</button>
...
function shOpen(i){
  ses = {live:true, mode:"shadow", pack:i, cards:SHADOW[i].l.map(l=>({en:l[0], az:l[1]})),
         i:0, rate:0.8, reps:0, tr:true};
  shadowStep();
}
function startShadow(){
  const p = shuffle(sentencePool()).slice(0,20);
  if (!p.length) return alert("Cümlə tapılmadı.");
  ses = {live:true, mode:"shadow", pack:null, cards:p, i:0, rate:0.8, reps:0, tr:true};
  shadowStep();
}
function shadowStep(){
  const s = ses;
  ...
  render(`
    ...
    <div class="spd">
      <button id="r0" class="${s.rate===0.55?"on":""}" onclick="SR(0.55)">Yavaş</button>
      <button id="r1" class="${s.rate===0.8?"on":""}" onclick="SR(0.8)">Orta</button>
...
function SR(r){ ses.rate=r; ["r0","r1","r2"].forEach((id,k)=>{ if($(id)) $(id).className=([0.55,0.8,1][k]===r?"on":""); }); SP(); }
...
function scrMe(){
  ...
  render(`
    ...
    <div class="menu">
      ...
      <button class="btn" onclick="voiceScreen()"><u><s>🗣️</s>Səs seçimi</u><i>${vA?esc(vA.name.slice(0,18)):"yoxdur"}</i></button>
...
/* ---------- ses secimi ---------- */
const VOICE_DEMO = "This is how I sound. Listen carefully and repeat after me.";
function voiceScreen(){
  pickVoices();
  head("Səs seçimi", voices.length ? voices.length+" səs" : "", null);
  let h = '<p class="note l" style="margin:0 0 14px">Sətrə toxun — həmin səs seçiləcək və ' +
          'nümunə cümlə səslənəcək. Seçim yadda saxlanılır.</p>';
  if (!voices.length){
    h += '<div class="gw">Telefonunda ingilis dili səsi tapılmadı. Ayarlar → Dil və giriş → ' +
         'Mətnin səsləndirilməsi bölməsindən English (UK) səs paketini endir.</div>';
  } else {
    h += voices.map((v,k)=>`<button class="les${v===vA?" read":""}" onclick="VPick(${k})">
      <n>${v===vA?"✓":""}</n><span>${esc(v.name)}</span><em>${esc(v.lang)}</em></button>`).join("");
  }
  render(h, null, true);
}
function VPick(k){
  const v = voices[k];
  if (!v) return;
  S.cfg.voice = String(v.voiceURI || v.name).slice(0, 120);
  save();
  pickVoices();
  voiceScreen();
  say(VOICE_DEMO);
